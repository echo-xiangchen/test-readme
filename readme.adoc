...
This repo contains a demo for running our APOC extension with CFG checking.

// tag::cfg-verificaiton[]
== CFG Verfication Path Finding 
// tag::cfg-verification-text[]
This is a fork version of the original https://github.com/neo4j-contrib/neo4j-apoc-procedures[APOC Github Project]. The reason why we did a fork was because to keep up with the Neo4J versions. It is important to note that for APOC to work properly, it needs to be consistent with the version of Neo4J. 

In our forked version of APOC, we included new functions to help construct paths in queries. Specifically, it verifies the feasibility of the paths using a CFG-based verification. 
...
./gradlew shadow
----
After the funning finished, go to the folder:
----
full/build/libs
----
You will find the `.jar` file named `apoc-4.4.0.8-all.jar`. This is the `.jar` file that we will use in Neo4j to call our queries.

Note: I do not use `gradle build` because it casues too much errors. Instead, I use `gradle shadow`. If you can figure something out with `gradle build`, feel free to do so.

// end::cfg-verification-text[]
// end::cfg-verification[]
// end::intro[]


// tag::set-up-neo4j[]
== Setting up Neo4j
First, you need to put the `.jar` file generated in the last step into the `plugins` folder before you start the Neo4j instance.

The Neo4j version we use is `4.4.X`, make sure you download the correct version.
...
== Running Demo
- Go to the folder `demo`, and run:
----
run-demo.sh
----
This script will remove unnecessary columns of the factbase, and simplify the Presence Conditions.
'''
- Copy the factbase from folder `2-simplify-conditions` into your Neo4j instance's `import` folder:
----
axtls-node.csv
axtls-edge.csv
----
'''
- Then run the following query to import the factbase into Neo4j:
[source,cypher,subs=attributes]
----
CALL apoc.import.csv(
  [{fileName: 'axtls-node.csv', labels: []}],
  [{fileName: 'axtls-edge.csv', type: ''}],
  {delimiter: '\t', arrayDelimiter: ':'});
----
'''
- Then run the following query to find the entry and exit point of each component:
[source,cypher,subs=attributes]
----
// var exit point of a comp
WITH "MATCH (a)-[:parWrite]->(b) WHERE a.comp <> b.comp RETURN DISTINCT a.id" AS query
CALL apoc.export.csv.query(query, "axtls-exitVar.csv", {})
YIELD file, source, format, nodes, relationships, properties, time, rows, batchSize, batches, done, data
RETURN file, source, format, nodes, relationships, properties, time, rows, batchSize, batches, done, data;

// var entry point of a comp
WITH "MATCH (a)-[:parWrite]->(b) WHERE a.comp <> b.comp RETURN DISTINCT b.id" AS query
CALL apoc.export.csv.query(query, "axtls-entryVar.csv", {})
YIELD file, source, format, nodes, relationships, properties, time, rows, batchSize, batches, done, data
RETURN file, source, format, nodes, relationships, properties, time, rows, batchSize, batches, done, data;

// func exit point of a comp
WITH "MATCH (f1:cFunction)-[:call]->(f2:cFunction) WHERE f1.comp <> f2.comp RETURN DISTINCT f1.id" AS query
CALL apoc.export.csv.query(query, "axtls-exitFunc.csv", {})
YIELD file, source, format, nodes, relationships, properties, time, rows, batchSize, batches, done, data
RETURN file, source, format, nodes, relationships, properties, time, rows, batchSize, batches, done, data;

// func entry point of a comp
WITH "MATCH (f1:cFunction)-[:call]->(f2:cFunction) WHERE f1.comp <> f2.comp RETURN DISTINCT f2.id" AS query
CALL apoc.export.csv.query(query, "axtls-entryFunc.csv", {})
YIELD file, source, format, nodes, relationships, properties, time, rows, batchSize, batches, done, data
RETURN file, source, format, nodes, relationships, properties, time, rows, batchSize, batches, done, data;
----
The output files will also be saved in the `import` folder.

'''
- Then copy the following files to our local folder `demo/3-gen-exit-entry-Var-Func`:
----
axtls-entryVar.csv
axtls-exitVar.csv
axtls-entryFunc.csv
axtls-exitFunc.csv
----
Then run the script to add entry and exit labels to corresponding nodes:
----
./run-add-labels.sh axtls
----
The modified `axtls-node.csv` now labels the nodes that are either an entry or exit point of a component.

'''
- Now go back to your Neo4j and update the database with the modified `axtls-node.csv` generated in the last step (you need to clean your database and re-import the updated factbase into Neo4j again).
- Run the following query:
----
MATCH (a:cVariable)
WITH *, apoc.cfgPath.nonRosFindPaths(a, {
    endN : a,
...
----
This query tries to find the triangle-shaped communication pattern with CFG checking.
// end::demo[]



= Below are the original APOC README content (you may find useful information below)
